...
def new_employee_alert(sheet):
    ...
    for row in sheet.iter_rows(min_row=2, max_col=hire_date_column_index, values_only=True):
        ...
        if (
            isinstance(hire_date, datetime.date)
            and hire_date.month == today.month
            and hire_date.year == today.year
        ):
    ...
    if hireDate_people:
        ...
        st.session_state.alerts.append(
            f"New employees in the current month in sheet '{sheet.title}' : \n{formatted_list}"
        )

    else:
        st.session_state.info.append("No new employees were found in the current month.")
